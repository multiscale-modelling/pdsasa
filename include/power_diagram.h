...
//			cout<<i<<" "<<vertices[i].endPoints[0]<<" "<<vertices[i].endPoints[1]<<" "<<vertices[i].endPoints[2]<<" "<<vertices[i].endPoints[3]<<endl;
	...
	void UpdateUnused()
	{
		...
		for(typename std::vector<vertexPtr>::iterator it=Replaced.begin();it!=Replaced.end();++it)
		{
			...
			{
				if((*it)-&vertices.front() < static_cast<int>(nRevertVertices))
        ...
        {
			for(typename std::vector<vertexPtr>::iterator it=Replaced.begin();it!=Replaced.end();++it)
				if((*it)-&vertices[0]>= static_cast<int>(nRevertVertices))
					unused.push_back(*it);
				else
...
template <class VectorSubtraction>
	PDCoord intersectionOfLineAndPlane3D(const VectorSubtraction& direction,const VectorSubtraction& supportVec,const VectorSubtraction& normal,const PDFloat& planeVal)
	{
		const PDFloat tmp=(normal.dot(direction));
//		const PDFloat sqr=normal.squaredNorm();

		if(tmp>powerErr*std::numeric_limits<PDFloat>::epsilon())
...
//			for(typename std::array<cellPtr,dimension+1>::iterator it=this->generators.begin()+dimension;it!=this->generators.begin();--it)
