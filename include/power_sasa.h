...
// Uncomment the next line to get correct volumes of individual atoms
#define ATOMVOL
...
public:
	...
	const std::vector<std::vector<PDCoord> >& getDSasa() const {return DSasa;}
...
private:
	...
	inline void Resize_NB(unsigned int nnb)
	{
		...
		if (withDSasa)
		{
		        for (unsigned int i = 0; i < DSasa.size(); ++i) DSasa[i].resize(nnb+1);
		}
...
//std::cout << "db> resize npnt=" << npnt << std::endl;
	...
	inline void Resize_NA()
	{
		...
		if(withDSasa)
		{
			unsigned int nnb = MAX_NB;
		        if (DSasa.size() > 0) nnb = DSasa[0].size() - 1;
		        unsigned int n_old = DSasa.size();
			DSasa.resize(n);
		        for (unsigned int i = n_old; i < n; ++i) DSasa[i].resize(nnb + 1);
		}
	...
	std::vector< std::vector<PDCoord> >   DSasa;
...
public:
	...
	{
		//if(!withVol)
			//exitFail("programmer error. Sasa defined without volume calculation");
		...
		std::cout<<"single sasa done"<<std::endl;
...
#endif
#endif //__APPLE__  This is a GCC 4.2 workaround (available on 10.6). Once 10.6 is out of support, remove it
};
//Implementation:
...
// ----- some initialisations  --------------------
	...
	if (withDSasa != 0) for (i = 0; i <= nnb; ++i) (DSasa)[iatom][i].setZero();
...
//			ip1 = br_p[iv][1];
		...
		do                                 // main loop
		{
			...
			if (withDSasa != 0) {
				...
				(DSasa)[iatom][ic1]+= ds1 * e[ic1] - ds2 * (*pt - *pt0).cross(e[ic1]);
			}
...
// ---------- sasa from single circles ----------------------------
	...
	for (i = 0; i < nnb; ++i)
	{
		...
		if (ok)
		{
			...
			if (withDSasa != 0)
			{
				DSasa[iatom][i]=( RAD*pi() * (1.0 + (nb_RAD2[i] - RAD2)/(nb_dist[i]*nb_dist[i])) ) * e[i];
			}
